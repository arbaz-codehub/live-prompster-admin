document.addEventListener('DOMContentLoaded', () => {
  ...
  window.loadJsonPayload = function(tabName) {
    const jsonStr = prompt("Paste the JSON payload generated by the Master Prompt:");
    if (!jsonStr) return;
    try {
      ...
      Object.keys(data).forEach(key => {
        ...
        if (input) {
          if (input.type === 'checkbox') {
            ...
            if (key === 'prompts') input.value = data[key].join('\n\n');
            else if (key === 'images' || key === 'screenshots' || key === 'why_not_buy') input.value = data[key].join('\n');
            else if (key === 'tags') input.value = data[key].join(', ');
          } else {
            input.value = data[key] !== null ? data[key] : '';
          }
        }
      });
      showToast(`JSON payload loaded for ${tabName}`, 'success');
    } catch (err) {
  ...
  setupFormHandler('promo', (data) => {
    ...
    // Delete frontend-only fields if they aren't in the DB to prevent crashes
  ...
  setupFormHandler('workshops', (data) => {
    ...
    data.fee_amount = parseFloat(data.fee_amount) || 0;
    return data;
  });
